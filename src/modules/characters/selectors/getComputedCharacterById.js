 ...
 */
const getComputedCharacterById = memoize(
    ...
    (state, id) => {
        ...
        const APMax = Math.round(
            ((end / 2) * (1 - (HPLost / HPMax)))
        );
        ...
        const hitChance = clamp(
            (Math.max(dex, int) / 20) + (per / 20 / 2),
            0, 1
        );
        const critChance = clamp(
            (Math.max(dex, int) / 20 / 2),
            0, 1
        );
        const defenseRating = clamp(
            (Math.max(dex, int) / 20 / 4),
            0, 1
        );
